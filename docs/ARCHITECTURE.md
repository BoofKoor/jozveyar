...
**وضعیت: تأییدنشده.** تا تأیید صاحب پروژه هیچ کدی نوشته نمی‌شود.
...
| کارگر اسناد | Python + LibreOffice + PyMuPDF + Ghostscript | همه‌چیز در Node با `node-canvas` |
...
| ذخیره‌سازی | S3 API؛ MinIO محلی → پارس‌پک یا آروان | دیسک محلی — آپلود از سرور رد می‌شد |
...
| پروکسی و TLS | Caddy | Nginx + certbot |
| تاریخ | `date-fns-jalali` | `dayjs` + `jalaliday` |
...
```
   ┌────────────┐  URL امضاشده   ┌──────────────────┐
   │  مرورگر    │◄──────────────►│ Caddy → web+admin│
   │  کاربر     │                │ Next.js، ۲ ساب‌دامین
   └─────┬──────┘                └────────┬─────────┘
         │ آپلود مستقیم                   │ سفارش و صف کار
         │ chunked · resumable            ▼
         │                       ┌──────────────────┐
         │                       │   PostgreSQL     │
         │                       │ سفارش، تحلیل، صف │
...
│  ├─ Caddyfile · deploy.sh · ship-image.sh
└─ docs/
...
| # | برش | محتوا |
|---|---|---|
| 0 | اسکلت قابل دیپلوی | Docker Compose، یک صفحه، یک مهاجرت، اسکریپت دیپلوی. هدف: روز اول ثابت کن خط دیپلوی کار می‌کند |
| 1 | **لحظهٔ جادو** | فایل بینداز ← مرورگر می‌خواند ← قیمت زنده با تعرفهٔ واقعی ← تنظیمات. بدون دیتابیس و حساب. + پایه‌های سئو (رندر ایستا، متا، نقشهٔ سایت، JSON-LD) |
| 2 | سرور منبع حقیقت | آپلود presigned و chunked، تحلیل کامل کارگر، هم‌ترازی قیمت، مسیر Word/PPT/عکس، ادغام PDF، هشدارها |
